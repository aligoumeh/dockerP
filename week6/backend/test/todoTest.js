...
describe('POST /api/todo/:message', function() {
  ...
  // INCOMPLETE!!! :-(
  it('should not allowed to give the same item again', function(done) {
    request(app)
        .post('/api/todo/TodoItem')
        .set('Accept', 'application/json')
        .expect(400, done);
  ...
  it('should not contain the item "TodoItem" in the list 2x', function(done) {
    request(app)
        .get('/api/todo')
        .set('Accept', 'application/json')
        .expect(200, ['TodoItem'], done);
...
describe('DELETE /api/todo/:message', function() {
  // INCOMPLETE!!! :-(
  it('shouldnt let to delete unexisting item from list', function(done) {
    request(app).delete('/api/todo/notExistingItem')
        .set('Accept', 'application/json')
        .expect(404, done);
  });
